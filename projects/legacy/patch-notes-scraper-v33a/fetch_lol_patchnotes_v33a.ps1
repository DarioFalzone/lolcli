...
$LogFile = Join-Path $LogDir ("fetch_patchnotes_v33a_{0}.log" -f $ts)
$Transcript = Join-Path $LogDir ("fetch_patchnotes_v33a_transcript_{0}.txt" -f $ts)
...
$LinkRx = [Regex]'(?is)href="(?<link>(?:https?://[^""]+)?/(?:es-es|es-mx|en-us)/news/game-updates/patch-[0-9\-]+-notes/?)"'
...
function Parse-PatchPage([string]$url,[int]$clip){
  ...
  if($m2.Success){ $published = $m2.Groups[1].Value }
  ...
  for($i=0; $i -lt $sections.Count; $i++){
    $sections[$i] | Add-Member -NotePropertyName end -NotePropertyValue ($i -lt $sections.Count-1 ? $sections[$i+1].start : $docLen) -Force
  }
  ...
  if ($champWin){
    ...
    for($i=0; $i -lt $h3.Count; $i++){
      ...
      $bend = ($i -lt $h3.Count-1) ? $h3[$i+1].Index : $frag.Length
      $block = $frag.Substring($bstart, [Math]::Max(0,$bend - $bstart))
...
try {
  ...
  Log ("Inicio V3.3a: N=$LatestN Out=$Out Dir=$ScriptDir Clip=$DetalleChars")
