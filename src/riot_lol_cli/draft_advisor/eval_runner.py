...
def _check_assertions(
    ...
    if allowed and top_id not in allowed:
        failures.append(f"Top pick '{top_id}' not in allowed set {allowed}")
    ...
    if must_include:
        ...
        if not any(c in top_3_ids for c in must_include):
            failures.append(f"Top 3 {top_3_ids} must include at least one of {must_include}")

    # must_not_recommend_as_top
    must_not = assertions.get("must_not_recommend_as_top", [])
    if top_id in must_not:
        failures.append(f"Top pick '{top_id}' is in must_not_recommend set {must_not}")

    # top_pick_score_min
    score_min = assertions.get("top_pick_score_min")
    if score_min is not None and top_score < score_min:
        failures.append(f"Top pick score {top_score:.1f} < minimum {score_min}")
    ...
    if cand_max is not None:
        ...
        if total > cand_max:
            failures.append(f"Total candidates {total} exceeds max {cand_max}")
...
def run_evaluation(
    ...
    force_fresh_adc_meta: bool = False,
    ...
    if force_fresh_adc_meta and data_service._adc_meta_snapshot:
    ...
    for case in cases:
        ...
        try:
            ...
            failures = _check_assertions(case_id, case["assertions"], recommendation)
            ...
            if failures:
                ...
                passed += 1
                logger.info(
                    f"  PASS (top: {recommendation.top_pick.id}, score: {recommendation.top_pick.total_score:.1f})"
                )
