...
class MetaDataCollectorDB:
    ...
    def __init__(
        self, api_key: str, platform: str = "la2", regional: str = "americas", db_path: str = "data/meta_analyzer.db"
    ):
        ...
        for participant in participants:
            champ_data = {
                ...
                "runes": {
                    ...
                    "secondary": participant.get("perks", {}).get("styles", [{}])[1].get("style")
                    if len(participant.get("perks", {}).get("styles", [])) > 1
                    else None,
                },
            }
    ...
    def get_hourly_stats(self, matches: Optional[list[dict]] = None, hours_back: int = 24) -> dict[str, dict]:
        ...
        try:
            ...
            raw_matches = session.query(RawMatch).filter(RawMatch.timestamp >= cutoff).all()

            # Agrupar por campeón
            champ_stats = defaultdict(
                lambda: {
                    "matches": 0,
                    "wins": 0,
                    "losses": 0,
    ...
    def get_stats_by_hour(self) -> dict:
        ...
        session = self.db.get_session()
        try:
            latest = session.query(ChampionHourly).order_by(ChampionHourly.hour_bucket.desc()).first()

            if not latest:
                return {}

            stats = {}
            hourly_stats = session.query(ChampionHourly).filter(ChampionHourly.hour_bucket == latest.hour_bucket).all()
