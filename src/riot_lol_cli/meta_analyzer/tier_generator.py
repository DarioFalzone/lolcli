...
class TierListGenerator:
    ...
    # Umbrales de tier
    TIER_THRESHOLDS = {Tier.S: 54.0, Tier.A: 51.5, Tier.B: 48.5, Tier.C: 46.0, Tier.D: 0.0}
    ...
    def generate_tier_list(
        self, stats: dict[str, Any], anomalies: Optional[list[Any]] = None
        ...
        tier_list.sort(key=lambda x: (list(Tier).index(x.tier), -x.winrate))
    ...
    def _detect_trend(self, champion: str, anomalies: list[Any]) -> Optional[str]:
        ...
        for anomaly in anomalies:
            if (
                anomaly.get("champion") == champion or anomaly.champion == champion
                if hasattr(anomaly, "champion")
                else False
            ):
    ...
    def _get_best_role(self, roles: dict[str, Any]) -> Optional[str]:
        ...
        best_role = max(roles.items(), key=lambda x: x[1].get("winrate", 0))

        return best_role[0] if best_role[1].get("matches", 0) > 20 else None

    def _generate_reason(self, champion: str, tier: Tier, winrate: float, trend: Optional[str]) -> str:
    ...
    def save_tier_list(self, tier_list: list[ChampionTierInfo], timestamp: Optional[str] = None) -> Path:
        """Guarda tier list"""
    ...
    def get_tier_list_html(self, tier_list: list[ChampionTierInfo]) -> str:
        ...
        for info in tier_list:
            trend_icon = {"RISING": "📈", "FALLING": "📉", None: "➡️"}.get(info.trend, "")
